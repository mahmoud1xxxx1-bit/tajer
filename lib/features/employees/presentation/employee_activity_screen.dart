...
class EmployeeActivityScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EmployeeActivityScreen> createState() =>
      _EmployeeActivityScreenState();
}

class _EmployeeActivityScreenState
    extends ConsumerState<EmployeeActivityScreen> {
  ...
  IconData _getIconForAction(String type, bool isSale) {
    ...
    if (type.contains('منتج') ||
        type.toLowerCase().contains('product') ||
        type.contains('بضاعة') ||
        type.contains('مخزون')) return Icons.inventory_2_outlined;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          isAr
              ? 'سجلات الموظف: ${widget.employeeName}'
              : 'Employee Activity: ${widget.employeeName}',
              ...
              builder: (context, snapshot) {
                ...
                final employeeOrders = allOrders
                    .where((o) =>
                        ...
                        (o.creatorName != null &&
                            o.creatorName == widget.employeeName &&
                ...
                final employeeExpenses = allExpenses
                    .where((e) =>
                        e.creatorId == widget.employeeId ||
                        (e.creatorName != null &&
                            e.creatorName == widget.employeeName &&
                            widget.employeeName.isNotEmpty))
                    .toList();
                    ...
                    try {
                      ...
                      if (doc.id.startsWith('act_')) {
                        ...
                        if (empId == widget.employeeId ||
                          ...
                          final actionRaw = data['actionType']?.toString() ??
                              (isAr ? 'عملية نظام' : 'System Action');
                          final action = actionRaw.contains('|')
                              ? (isAr
                                  ? actionRaw.split('|')[1]
                                  : actionRaw.split('|')[0])
                              : actionRaw;
                          ...
                          final subtitle = descRaw.contains('|')
                              ? (isAr
                                  ? descRaw.split('|')[1]
                                  : descRaw.split('|')[0])
                              : descRaw;
                        ...
                        if (empEmail == widget.employeeName ||
                            empEmail.contains(widget.employeeName)) {
                    ...
                    return GlassCard(
                      ...
                      child: ClipRRect(
                        ...
                        child: ExpansionTile(
                          ...
                          children: dayItems
                              .map((item) =>
                                  _buildLogItem(item, theme, currency, isAr))
                              .toList(),
                        ),
                      ),
                    );
  ...
  Widget _buildLogItem(
      EmployeeActivityItem item, ThemeData theme, dynamic currency, bool isAr) {
    return InkWell(
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: Icon(_getIconForAction(item.actionType, item.isSale),
                  color: item.badgeColor, size: 24),
            ),
