...
class _SuppliersScreenState extends ConsumerState<SuppliersScreen> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: FirestoreListView<Supplier>(
              query: query,
              pageSize: 30,
  ...
  Widget _buildSupplierCard(Supplier supplier, BuildContext context, WidgetRef ref, String currency, bool canManageSuppliers) {
    ...
    return GlassCard(
      ...
      onTap: () {
        Navigator.push(context, MaterialPageRoute(builder: (_) => SupplierDetailsScreen(supplier: supplier)));
      },
      child: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text(supplier.phone, style: const TextStyle(color: Colors.grey, fontFamily: 'Tajawal')),
                    ],
                  ),
                ...
                if (canManageSuppliers)
                  PopupMenuButton<String>(
                    ...
                    onSelected: (value) async {
                      if (value == 'edit') {
                        showModalBottomSheet(
                          ...
                          builder: (context) => Padding(
                            padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom),
                            child: AddSupplierDialog(supplierToEdit: supplier),
                          ),
                        );
                        ...
                        if (appUser != null) {
                          ...
                          final success = await PinConfirmationDialog.requirePinOrSetup(context, appUser, title: title, warning: warning);
                          if (!success) return;
                        }
                        try {
                    ...
                    itemBuilder: (context) => [
                      PopupMenuItem(
                        value: 'edit',
                        child: Row(children: [const Icon(Icons.edit, size: 20, color: Colors.blue), const SizedBox(width: 8), Text(l10n.edit, style: const TextStyle(fontFamily: 'Tajawal'))]),
                      ),
                      PopupMenuItem(
                        value: 'delete',
                        child: Row(
                          children: [
                            ...
                            Text(
                              supplier.isActive
                                  ? (Localizations.localeOf(context).languageCode == 'ar' ? 'إلغاء المورد' : 'Cancel Supplier')
                                  : (Localizations.localeOf(context).languageCode == 'ar' ? 'استعادة المورد' : 'Restore Supplier'),
