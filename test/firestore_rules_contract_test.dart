...
void main() {
  group('Multi-branch Firestore security contract', () {
    ...
    test('customer debt provenance is explicitly permissioned', () {
      ...
      expect(rules, contains("allow create: if hasPermission(merchantId, 'can_receive_payments');"));
    ...
    test('customer financial snapshots cannot grant arbitrary profile edits', () {
      ...
      expect(rules, contains("request.resource.data.diff(resource.data).affectedKeys().hasOnly(["));
    ...
    test('inventory audit log is append-only for cashiers', () {
      ...
      expect(rules, contains("allow create: if hasPermission(merchantId, 'can_manage_inventory') || hasPermission(merchantId, 'can_create_orders');"));
      expect(rules, contains("allow update, delete: if hasPermission(merchantId, 'can_manage_inventory');"));
    });

    test('inventory transfer provenance is manager-created and owner-mutable only', () {
      expect(rules, contains('match /inventory_transfers/{transferId}'));
      expect(rules, contains("allow create: if hasPermission(merchantId, 'can_manage_inventory');"));
      expect(rules, contains('allow update, delete: if isOwner(merchantId);'));
    });
